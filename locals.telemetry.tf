...
locals {
  ...
  telem_workload_deploy = local.disable_telemetry ? 1 : 0
}
...
locals {
  telem_workload_bitfield_telemetry = (
    local.telem_workload_deploy
  )
  telem_workload_bitfield_hex = format("%04x", local.telem_workload_bitfield_telemetry)
}
...
locals {
  telem_workload_arm_deployment_name = substr(
    format(
      "pid-%s_%s_%s_%s",
      local.telem_workload_puid,
      local.org_name,
      local.telem_workload_bitfield_hex,
...
# Condition to determine whether we create the workload telemetry deployment
locals {
  telem_workload_deployment_enabled = !local.disable_telemetry
}
